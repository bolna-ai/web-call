...
var WebCallError = class extends Error {
  constructor(code, message, scope, cause2) {
    super(message);
    this.code = code;
    this.scope = scope;
    this.cause2 = cause2;
...
var BolnaWebCall = class extends Emitter {
  constructor(options) {
    super();
    this.state = "idle";
    this.runId = null;
  ...
  async fetchSession(userData) {
    ...
    try {
      ...
      if (session) {
        this.options = { ...this.options, session: void 0 };
        return session;
      }
  ...
  // media
  // ------------------------------------------------------------------
  async preflightMicrophone(constraints) {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: constraints, video: false });
      stream.getTracks().forEach((track) => track.stop());
    } catch (err) {
      throw new WebCallError("microphone_denied", "Microphone access was denied", void 0, err);
    }
  }
  ensureAudioElement() {
  ...
  // signaling
  // ------------------------------------------------------------------
  async connect(session, audioConstraints) {
    ...
    try {
      ...
      throw new WebCallError("connect_failed", "Could not connect to the call server", void 0, err);
    }
    ...
    await new Promise((resolve, reject) => {
      ...
      inviter.stateChange.addListener((state) => {
        ...
        if (state === SessionState2.Establishing) {
          this.setState("ringing");
        } else if (state === SessionState2.Established) {
        ...
        } else if (state === SessionState2.Terminated) {
          if (!settled) {
            settled = true;
            clearTimeout(timeout);
      ...
      inviter.invite().catch((err) => {
        ...
        settled = true;
        clearTimeout(timeout);
        reject(new WebCallError("connect_failed", "INVITE failed", void 0, err));
      });
    });
  }
  peerConnection() {
    const handler = this.inviter?.sessionDescriptionHandler;
  ...
  async sendHangup() {
    ...
    if (!inviter) return;
    try {
      if (inviter.state === SessionState2.Established) await inviter.bye();
      else if (inviter.state === SessionState2.Establishing) await inviter.cancel();
    } catch {
    }
  }
  // ------------------------------------------------------------------
